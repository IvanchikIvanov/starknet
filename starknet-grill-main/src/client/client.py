...
class Client:
    ...
    async def _approve(self, token_address, spender, amount) -> bool:
        ...
        tx = await prepared_tx.invoke(max_fee=int(fee * (1.75 + randint(RANDOM_INCREASE_FEE[0], RANDOM_INCREASE_FEE[1]) / 100)))
        for _ in range(150):
            try:
    ...
    async def call(self, interacted_contract_address, calldata, selector_name):
        for _ in range(5):
            try:
                ...
                resp = await self.account.execute(calls=[call], max_fee=int(max_fee.Wei * (1.75 + randint(RANDOM_INCREASE_FEE[0], RANDOM_INCREASE_FEE[1]) / 100)))
                for _ in range(300):
                    try:
    ...
    async def multicall(self, calls_data):
        try:
            logger.debug(f"[{self.address}] Sending tx...")
            max_fee = TokenAmount(amount=float(uniform(0.0007534534534, 0.001)))
            await self.estimate_fee()
            resp = await self.account.execute(calls=calls_data, max_fee=int(max_fee.Wei * (1.75 + randint(RANDOM_INCREASE_FEE[0], RANDOM_INCREASE_FEE[1]) / 100)))
            for _ in range(300):
                try:
                    receipt = await self.account.client.get_transaction_receipt(resp.transaction_hash)
                    block = receipt.block_number
    ...
    async def send_eth(self, to_address):
        eth_data = DefaultContractData.ETH_CONTRACT
        eth_contract = Contract(address=eth_data.get('address'), abi=eth_data.get('abi'), provider=self.account)
        amount = await self.get_balance(token_address=eth_data.get('address'))
        amount_to_send = TokenAmount(amount=float(amount.Ether) - float(uniform(AMOUNT_TO_LEFT_ON_BALANCE[0], AMOUNT_TO_LEFT_ON_BALANCE[1])), decimals=18)
        logger.debug(f"Sending {amount_to_send.Ether} ETH to {to_address}")
        send_tx = await self.send_transaction(interacted_contract=eth_contract,
