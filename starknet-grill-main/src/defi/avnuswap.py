...
class AvnuSwap:
    ...
    async def refuel(self, token_address, amount: TokenAmount):
        token_data = DefaultContractData.get_data_from_contract(token_address)
        token_name = token_data.get('name')
        decimals = token_data.get('decimals')
        try:
            balance = await self.client.get_balance(token_address=token_address, decimals=decimals)
            logger.debug(
                f"[{self.client.address}] Refueling from | [AvnuSwap] | Current balance: {token_name} {balance.Ether}")
            eth_price = self.client.get_eth_price()
            eth_amount = TokenAmount(amount=float(amount.Ether) / eth_price, decimals=18)
            if await self.swap(data_for_swap={"amount": eth_amount,
                                              'to_token_address': token_address,
                                              'to_token_name': token_data.get('name'),
                                              'from_token_address': DefaultContractData.ETH_CONTRACT.get('address'),
                                              'from_token_name': 'ETH',
            ...
            logger.error(f"[{self.client.address}] Couldn't refuel | [AvnuSwap] | {exc}")
