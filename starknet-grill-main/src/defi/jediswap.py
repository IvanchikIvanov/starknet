...
from src.config import SLIPPAGE, DELAY_BETWEEN_ACTIONS, PERCENTAGE_BALANCE_FOR_SWAP
...
class JediSwap:
    ...
    async def add_liquidity(self) -> bool:
        try:
            ...
            balanceOne = await self.client.get_balance(token_address=token_one_address, decimals=token_one_decimals)
            balanceTwo = await self.client.get_balance(token_address=token_two_address, decimals=token_two_decimals)

            if amount_in_usdt < 0.1:
                amount_in_usdt = float((await self.client.get_balance()).Ether) * (self.client.get_eth_price()) * (random.randint(PERCENTAGE_BALANCE_FOR_SWAP[0], PERCENTAGE_BALANCE_FOR_SWAP[1])/100)
                ROUTERS_CHOISE = [AvnuSwap(client=self.client).refuel]
                if token_one_name != 'ETH':
                    refuel_one = await random.choice(ROUTERS_CHOISE)(token_address=token_one_address, amount=TokenAmount(amount=amount_in_usdt, decimals=token_one_decimals))
                    random_sleep = random.randint(DELAY_BETWEEN_ACTIONS[0], DELAY_BETWEEN_ACTIONS[1])
                    logger.info(
                        f"[{self.client.address}] Sleeping for {random_sleep} sec after refueling | [JediSwap]")
                    await asyncio.sleep(random_sleep)
                refuel_two = await random.choice(ROUTERS_CHOISE)(token_address=token_two_address,
                                                                 amount=TokenAmount(amount=amount_in_usdt,
                                                                                    decimals=token_two_decimals))
                random_sleep = random.randint(DELAY_BETWEEN_ACTIONS[0], DELAY_BETWEEN_ACTIONS[1])
                logger.info(
                    f"[{self.client.address}] Sleeping for {random_sleep} sec after refueling | [JediSwap]")
                await asyncio.sleep(random_sleep)
                ...
                if token_one_name == 'ETH':
                    amount_one = TokenAmount(amount=amount_in_usdt/self.client.get_eth_price(), decimals=18)
                else:
                    amount_one = TokenAmount(amount=amount_in_usdt, decimals=token_one_decimals)
                amount_two = TokenAmount(amount=amount_in_usdt, decimals=token_two_decimals)
            ...
            if is_approved_one and is_approved_two:
                ...
                if tx_hash:
                    ...
                    for _ in range(5):
                        try:
                            if await self.remove_liquidity(pooled_token_contract=pooled_token_address,
