from src.utils.utils import ACCOUNTS_LIST, PROXY_ADDRESSES_LIST
...
async def start():
    if not NODE_URL:
        ...
        return
    if USE_PROXY and len(PROXY_ADDRESSES_LIST) != len(ACCOUNTS_LIST):
        logger.critical(f"Add proxy to proxy.txt file.\nYour proxy quantity: {len(PROXY_ADDRESSES_LIST)}\nYour accounts quantity: {len(ACCOUNTS_LIST)}")
        return
